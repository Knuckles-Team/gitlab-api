[Skip to main content](https://docs.gitlab.com/api/events/#skipTarget) [Go to GitLab Docs homepage](https://docs.gitlab.com/)
`/`
...
v18.10
  *     * [18.10 (not yet released)](https://docs.gitlab.com/api/events/)
  *     * [18.9 (recently released)](https://docs.gitlab.com/18.9/api/events/)
    * [18.8](https://docs.gitlab.com/18.8/api/events/)
    * [18.7](https://archives.docs.gitlab.com/18.7/api/events/)
  *     * [17.11](https://archives.docs.gitlab.com/17.11/api/events/)
    * [16.11](https://archives.docs.gitlab.com/16.11/ee/api/events.html)
...
/
  ...
  5. [Events](https://docs.gitlab.com/api/events/)
...
For information about activity retention limits, see:
  * [User activity time period limit](https://docs.gitlab.com/user/profile/contributions_calendar/#event-time-period-limit)
  * [Project activity time period limit](https://docs.gitlab.com/user/project/working_with_projects/#view-project-activity)
...
This API has limitations related to epics, merge requests, and bulk push events:
  ...
  * Some merge request notes may instead use the `DiscussionNote` type. This target type is [not supported by the API](https://docs.gitlab.com/api/discussions/#understand-note-types-in-the-api).
  * Bulk push events created when a push exceeds the [Push event activities limit](https://docs.gitlab.com/administration/settings/push_event_activities_limit/) are returned with limited details: `commit_count: 0`, `ref_count` showing the number of refs pushed, and `null` values for individual commit attributes (`commit_from`, `commit_to`, `ref`, `commit_title`).


## List all events[](https://docs.gitlab.com/api/events/#list-all-events "Permalink")
Lists all events for the currently authenticated user. Does not return events associated with epics or merge requests. Returns bulk push events with limited commit details.
Prerequisites:
...
---|---|---|---
`action` | string | no | If defined, returns events with the specified [action type](https://docs.gitlab.com/user/profile/contributions_calendar/#user-contribution-events).
...
## Get contribution events for a user[](https://docs.gitlab.com/api/events/#get-contribution-events-for-a-user "Permalink")
Gets the contribution events for a specified user. Does not return events associated with epics or merge requests. Returns bulk push events with limited commit details.
Prerequisites:
  * Your access token must have either the `read_user` or `api` scope.
...
## List all visible events for a project[](https://docs.gitlab.com/api/events/#list-all-visible-events-for-a-project "Permalink")
Lists all visible events for a specified project. Returns bulk push events created when a push exceeds the [Push event activities limit](https://docs.gitlab.com/administration/settings/push_event_activities_limit/) with limited commit details: `commit_count: 0`, `ref_count` showing the number of refs pushed, and `null` values for individual commit attributes.
```
...
Parameter | Type | Required | Description
---|---|---|---
`project_id` | integer or string | yes | ID or [URL-encoded path](https://docs.gitlab.com/api/rest/#namespaced-paths) of a project.
`action` | string | no | If defined, returns events with the specified [action type](https://docs.gitlab.com/user/profile/contributions_calendar/#user-contribution-events).
`target_type` | string | no | If defined, returns the specified events. Possible values: `epic`, `issue`, `merge_request`, `milestone`, `note`, `project`, `snippet`, and `user`.
`before` | date (ISO 8601) | no | If defined, returns events created before the specified date.
`after` | date (ISO 8601) | no | If defined, returns events created after the specified date.
...
Edit this page
  *     * [ Open in Web IDE`.`Quickly and easily edit multiple files. ](https://gitlab.com/-/ide/project/gitlab-org/gitlab/edit/master/-/doc/api/events.md)
    * [ View page sourceEdit this file only. ](https://gitlab.com/gitlab-org/gitlab/-/blob/master/doc/api/events.md)
    * [ Create an issueSuggest improvements. ](https://gitlab.com/gitlab-org/gitlab/-/issues/new?issuable_template=Documentation)


  * [List all events](https://docs.gitlab.com/api/events/#list-all-events)
  * [Get contribution events for a user](https://docs.gitlab.com/api/events/#get-contribution-events-for-a-user)
  * [List all visible events for a project](https://docs.gitlab.com/api/events/#list-all-visible-events-for-a-project)
...
Feedback
  * [View page source](https://gitlab.com/gitlab-org/gitlab/-/blob/master/doc/api/events.md)
  * [Edit in Web IDE](https://gitlab.com/-/ide/project/gitlab-org/gitlab/edit/master/-/doc/api/events.md)
...
![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=10503924d6784ad3f2d352558587a67a&_biz_l=https%3A%2F%2Fdocs.gitlab.com%2Fapi%2Fevents%2F&_biz_t=1772174268675&_biz_i=Events%20API%20%7C%20GitLab%20Docs&_biz_n=48&rnd=257150&cdn_o=a&_biz_z=1772174268676)
