[Skip to main content](https://docs.gitlab.com/api/issues/#skipTarget) [Go to GitLab Docs homepage](https://docs.gitlab.com/)
`/`
...
v18.10
  *     * [18.10 (not yet released)](https://docs.gitlab.com/api/issues/)
  *     * [18.9 (recently released)](https://docs.gitlab.com/18.9/api/issues/)
    * [18.8](https://docs.gitlab.com/18.8/api/issues/)
    * [18.7](https://archives.docs.gitlab.com/18.7/api/issues/)
  *     * [17.11](https://archives.docs.gitlab.com/17.11/api/issues/)
    * [16.11](https://archives.docs.gitlab.com/16.11/ee/api/issues.html)
...
/
  ...
  5. [Issues](https://docs.gitlab.com/api/issues/)
...
Use this API to manage [issues](https://docs.gitlab.com/user/project/issues/). You can:
...
Responses in this API are [paginated](https://docs.gitlab.com/api/rest/#pagination) and return 20 results by default.
...
## List all issues[](https://docs.gitlab.com/api/issues/#list-all-issues "Permalink")
...
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
## List all group issues[](https://docs.gitlab.com/api/issues/#list-all-group-issues "Permalink")
Lists all issues for a specified group.
If the group is private, you must provide credentials to authorize. The preferred way to do this, is by using [personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/).
```
...
Attribute | Type | Required | Description
---|---|---|---
`id` | integer or string | Yes | The global ID or [URL-encoded path](https://docs.gitlab.com/api/rest/#namespaced-paths) of the group.
`assignee_id` | integer | No | Return issues assigned to the given user `id`. Mutually exclusive with `assignee_username`. `None` returns unassigned issues. `Any` returns issues with an assignee.
`assignee_username` | string array | No | Return issues assigned to the given `username`. Similar to `assignee_id` and mutually exclusive with `assignee_id`. In GitLab CE, the `assignee_username` array should only contain a single value. Otherwise, an invalid parameter error is returned. Only issues assigned to all passed users are returned.
`author_id` | integer | No | Return issues created by the given user `id`. Mutually exclusive with `author_username`. Combine with `scope=all` or `scope=assigned_to_me`.
`author_username` | string | No | Return issues created by the given `username`. Similar to `author_id` and mutually exclusive with `author_id`.
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
## List all project issues[](https://docs.gitlab.com/api/issues/#list-all-project-issues "Permalink")
History
...
If the project is private, you need to provide credentials to authorize. The preferred way to do this, is by using [personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/).
```
...
Attribute | Type | Required | Description
---|---|---|---
`id` | integer or string | Yes | The global ID or [URL-encoded path of the project](https://docs.gitlab.com/api/rest/#namespaced-paths).
`assignee_id` | integer | No | Return issues assigned to the given user `id`. Mutually exclusive with `assignee_username`. `None` returns unassigned issues. `Any` returns issues with an assignee.
`assignee_username` | string array | No | Return issues assigned to the given `username`. Similar to `assignee_id` and mutually exclusive with `assignee_id`. In GitLab CE, the `assignee_username` array should only contain a single value. Otherwise, an invalid parameter error is returned. Only issues assigned to all passed users are returned.
`author_id` | integer | No | Return issues created by the given user `id`. Mutually exclusive with `author_username`. Combine with `scope=all` or `scope=assigned_to_me`.
`author_username` | string | No | Return issues created by the given `username`. Similar to `author_id` and mutually exclusive with `author_id`.
...
This endpoint supports both offset-based and [keyset-based](https://docs.gitlab.com/api/rest/#keyset-based-pagination) pagination. You should use keyset-based pagination when requesting consecutive pages of results.
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
## Retrieve an issue[](https://docs.gitlab.com/api/issues/#retrieve-an-issue "Permalink")
Only for administrators.
Retrieves a specified issue.
The preferred way to do this is by using [personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/).
```
GET /issues/:id
```
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated, and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
## Retrieve a project issue[](https://docs.gitlab.com/api/issues/#retrieve-a-project-issue "Permalink")
Retrieves a specified issue for a project.
If the project is private or the issue is confidential, you need to provide credentials to authorize. The preferred way to do this, is by using [personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/).
```
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
## Create an issue[](https://docs.gitlab.com/api/issues/#create-an-issue "Permalink")
...
`milestone_id` | integer | No | The global ID of a milestone to assign issue. To find the `milestone_id` associated with a milestone, view an issue with the milestone assigned and [use the API](https://docs.gitlab.com/api/issues/#retrieve-a-project-issue) to retrieve the issue’s details.
...
If the target project has **Issues** [turned off](https://docs.gitlab.com/user/project/settings/#toggle-project-features), you get the `403` response, along with the message:
json
```
{
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
### Rate limits[](https://docs.gitlab.com/api/issues/#rate-limits "Permalink")
To help avoid abuse, users can be limited to a specific number of `Create` requests per minute. See [Issues rate limits](https://docs.gitlab.com/administration/settings/rate_limit_on_issues_creation/).
## Update an issue[](https://docs.gitlab.com/api/issues/#update-an-issue "Permalink")
...
## Delete an issue[](https://docs.gitlab.com/api/issues/#delete-an-issue "Permalink")
History
...
## Reorder an issue[](https://docs.gitlab.com/api/issues/#reorder-an-issue "Permalink")
Reorders a specified issue within a project. You can see the results when [sorting issues manually](https://docs.gitlab.com/user/project/issues/sorting_issue_lists/#manual-sorting).
```
...
## Move an issue[](https://docs.gitlab.com/api/issues/#move-an-issue "Permalink")
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
## Clone an issue[](https://docs.gitlab.com/api/issues/#clone-an-issue "Permalink")
...
`with_notes` | boolean | No | Clone the issue with [notes](https://docs.gitlab.com/api/notes/). Default is `false`.
Example request:
shell
```
curl --request POST \
...
## Notifications[](https://docs.gitlab.com/api/issues/#notifications "Permalink")
The following requests are related to [email notifications](https://docs.gitlab.com/user/profile/notifications/) for issues.
### Subscribe to an issue[](https://docs.gitlab.com/api/issues/#subscribe-to-an-issue "Permalink")
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
The `epic_iid` attribute is deprecated and [scheduled for removal](https://gitlab.com/gitlab-org/gitlab/-/issues/35157) in API version 5. Use `iid` of the `epic` attribute instead.
### Unsubscribe from an issue[](https://docs.gitlab.com/api/issues/#unsubscribe-from-an-issue "Permalink")
...
## Create a to-do item for an issue[](https://docs.gitlab.com/api/issues/#create-a-to-do-item-for-an-issue "Permalink")
...
The `assignee` column is deprecated. We now show it as a single-sized array `assignees` to conform to the GitLab EE API.
## Promote an issue to an epic[](https://docs.gitlab.com/api/issues/#promote-an-issue-to-an-epic "Permalink")
  * Tier: Premium, Ultimate
  * Offering: GitLab.com, GitLab Self-Managed, GitLab Dedicated


Promotes a specified issue to an epic by adding a comment with the [`/promote_to` quick action](https://docs.gitlab.com/user/project/quick_actions/#promote_to).
For more information about promoting issues to epics, see [Promote an issue to an epic](https://docs.gitlab.com/user/project/issues/managing_issues/#promote-an-issue-to-an-epic).
```
...
## Time tracking[](https://docs.gitlab.com/api/issues/#time-tracking "Permalink")
The following requests are related to [time tracking](https://docs.gitlab.com/user/project/time_tracking/) on issues.
### Set a time estimate for an issue[](https://docs.gitlab.com/api/issues/#set-a-time-estimate-for-an-issue "Permalink")
...
### Reset the time estimate for an issue[](https://docs.gitlab.com/api/issues/#reset-the-time-estimate-for-an-issue "Permalink")
...
### Add spent time for an issue[](https://docs.gitlab.com/api/issues/#add-spent-time-for-an-issue "Permalink")
...
### Reset spent time for an issue[](https://docs.gitlab.com/api/issues/#reset-spent-time-for-an-issue "Permalink")
...
### Retrieve time tracking stats for an issue[](https://docs.gitlab.com/api/issues/#retrieve-time-tracking-stats-for-an-issue "Permalink")
Retrieves time tracking stats for a specified issue in human-readable format (for example, `1h30m`) and in number of seconds.
If the project is private or the issue is confidential, you must provide credentials to authorize. The preferred way to do this, is by using [personal access tokens](https://docs.gitlab.com/user/profile/personal_access_tokens/).
```
...
## Merge requests[](https://docs.gitlab.com/api/issues/#merge-requests "Permalink")
The following requests are related to relationships between issues and merge requests.
### List all merge requests related to an issue[](https://docs.gitlab.com/api/issues/#list-all-merge-requests-related-to-an-issue "Permalink")
...
### List all merge requests that close an issue on merge[](https://docs.gitlab.com/api/issues/#list-all-merge-requests-that-close-an-issue-on-merge "Permalink")
...
## List all participants in an issue[](https://docs.gitlab.com/api/issues/#list-all-participants-in-an-issue "Permalink")
...
## Comments on issues[](https://docs.gitlab.com/api/issues/#comments-on-issues "Permalink")
Interact with comments using the [Notes API](https://docs.gitlab.com/api/notes/).
## Retrieve user agent details for an issue[](https://docs.gitlab.com/api/issues/#retrieve-user-agent-details-for-an-issue "Permalink")
...
## List issue state events[](https://docs.gitlab.com/api/issues/#list-issue-state-events "Permalink")
To track which state was set, who did it, and when it happened, use [Resource state events API](https://docs.gitlab.com/api/resource_state_events/#issues).
## Incidents[](https://docs.gitlab.com/api/issues/#incidents "Permalink")
The following requests are available only for [incidents](https://docs.gitlab.com/operations/incident_management/incidents/).
### Upload a metric image for an incident[](https://docs.gitlab.com/api/issues/#upload-a-metric-image-for-an-incident "Permalink")
Available only for [incidents](https://docs.gitlab.com/operations/incident_management/incidents/).
Uploads a screenshot of metric charts to display in a specified incident’s **Metrics** tab. When you upload an image, you can associate the image with text or a link to the original graph. If you add a URL, you can access the original graph by selecting the hyperlink above the uploaded image.
```
...
### List all metric images for an incident[](https://docs.gitlab.com/api/issues/#list-all-metric-images-for-an-incident "Permalink")
Available only for [incidents](https://docs.gitlab.com/operations/incident_management/incidents/).
...
### Update a metric image for an incident[](https://docs.gitlab.com/api/issues/#update-a-metric-image-for-an-incident "Permalink")
Available only for [incidents](https://docs.gitlab.com/operations/incident_management/incidents/).
...
### Delete a metric image from an incident[](https://docs.gitlab.com/api/issues/#delete-a-metric-image-from-an-incident "Permalink")
Available only for [incidents](https://docs.gitlab.com/operations/incident_management/incidents/).
...
Edit this page
  *     * [ Open in Web IDE`.`Quickly and easily edit multiple files. ](https://gitlab.com/-/ide/project/gitlab-org/gitlab/edit/master/-/doc/api/issues.md)
    * [ View page sourceEdit this file only. ](https://gitlab.com/gitlab-org/gitlab/-/blob/master/doc/api/issues.md)
  ...
  * [List all group issues](https://docs.gitlab.com/api/issues/#list-all-group-issues)
  * [List all project issues](https://docs.gitlab.com/api/issues/#list-all-project-issues)
  * [Retrieve an issue](https://docs.gitlab.com/api/issues/#retrieve-an-issue)
  * [Retrieve a project issue](https://docs.gitlab.com/api/issues/#retrieve-a-project-issue)
  * [Create an issue](https://docs.gitlab.com/api/issues/#create-an-issue)
  * [Rate limits](https://docs.gitlab.com/api/issues/#rate-limits)
  * [Update an issue](https://docs.gitlab.com/api/issues/#update-an-issue)
  * [Delete an issue](https://docs.gitlab.com/api/issues/#delete-an-issue)
  * [Reorder an issue](https://docs.gitlab.com/api/issues/#reorder-an-issue)
  * [Move an issue](https://docs.gitlab.com/api/issues/#move-an-issue)
  * [Clone an issue](https://docs.gitlab.com/api/issues/#clone-an-issue)
  * [Notifications](https://docs.gitlab.com/api/issues/#notifications)
  * [Subscribe to an issue](https://docs.gitlab.com/api/issues/#subscribe-to-an-issue)
  * [Unsubscribe from an issue](https://docs.gitlab.com/api/issues/#unsubscribe-from-an-issue)
  * [Create a to-do item for an issue](https://docs.gitlab.com/api/issues/#create-a-to-do-item-for-an-issue)
  * [Promote an issue to an epic](https://docs.gitlab.com/api/issues/#promote-an-issue-to-an-epic)
  * [Time tracking](https://docs.gitlab.com/api/issues/#time-tracking)
  * [Set a time estimate for an issue](https://docs.gitlab.com/api/issues/#set-a-time-estimate-for-an-issue)
  * [Reset the time estimate for an issue](https://docs.gitlab.com/api/issues/#reset-the-time-estimate-for-an-issue)
  * [Add spent time for an issue](https://docs.gitlab.com/api/issues/#add-spent-time-for-an-issue)
  * [Reset spent time for an issue](https://docs.gitlab.com/api/issues/#reset-spent-time-for-an-issue)
  * [Retrieve time tracking stats for an issue](https://docs.gitlab.com/api/issues/#retrieve-time-tracking-stats-for-an-issue)
  * [Merge requests](https://docs.gitlab.com/api/issues/#merge-requests)
  * [List all merge requests related to an issue](https://docs.gitlab.com/api/issues/#list-all-merge-requests-related-to-an-issue)
  * [List all merge requests that close an issue on merge](https://docs.gitlab.com/api/issues/#list-all-merge-requests-that-close-an-issue-on-merge)
  * [List all participants in an issue](https://docs.gitlab.com/api/issues/#list-all-participants-in-an-issue)
  * [Comments on issues](https://docs.gitlab.com/api/issues/#comments-on-issues)
  * [Retrieve user agent details for an issue](https://docs.gitlab.com/api/issues/#retrieve-user-agent-details-for-an-issue)
  * [List issue state events](https://docs.gitlab.com/api/issues/#list-issue-state-events)
  * [Incidents](https://docs.gitlab.com/api/issues/#incidents)
  * [Upload a metric image for an incident](https://docs.gitlab.com/api/issues/#upload-a-metric-image-for-an-incident)
  * [List all metric images for an incident](https://docs.gitlab.com/api/issues/#list-all-metric-images-for-an-incident)
  * [Update a metric image for an incident](https://docs.gitlab.com/api/issues/#update-a-metric-image-for-an-incident)
  * [Delete a metric image from an incident](https://docs.gitlab.com/api/issues/#delete-a-metric-image-from-an-incident)
...
Feedback
  * [View page source](https://gitlab.com/gitlab-org/gitlab/-/blob/master/doc/api/issues.md)
  * [Edit in Web IDE](https://gitlab.com/-/ide/project/gitlab-org/gitlab/edit/master/-/doc/api/issues.md)
...
![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=6cd0908ec84a42fec9a17ce4b3f900fc&_biz_l=https%3A%2F%2Fdocs.gitlab.com%2Fapi%2Fimport%2F&_biz_t=1771981559772&_biz_i=Import%20API%20%7C%20GitLab%20Docs&_biz_n=70&rnd=524581&cdn_o=a&_biz_z=1771981559874)![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=6cd0908ec84a42fec9a17ce4b3f900fc&_biz_l=https%3A%2F%2Fdocs.gitlab.com%2Fapi%2Fgroup_wikis%2F&_biz_t=1771981559822&_biz_i=Group%20wikis%20API%20%7C%20GitLab%20Docs&_biz_n=71&rnd=826523&cdn_o=a&_biz_z=1771981559874)![](https://cdn.bizible.com/ipv?_biz_r=&_biz_h=800054037&_biz_u=6cd0908ec84a42fec9a17ce4b3f900fc&_biz_l=https%3A%2F%2Fdocs.gitlab.com%2Fapi%2Fissues%2F&_biz_t=1771981559873&_biz_i=Issues%20API%20%7C%20GitLab%20Docs&_biz_n=72&rnd=324924&cdn_o=a&_biz_z=1771981559874)
