...
def mock_requests():
    def mock_side_effect(*args, **kwargs):
        ...
        if any(url.endswith(x) for x in ["/jobs", "/pipelines", "/releases", "/projects", "/groups", "/members"]):
            data = [{"id": 1, "name": "test"}] # Return at least one item with ID
    ...
    with patch("requests.Session.get", side_effect=mock_side_effect) as mock_get, patch(
        "requests.Session.post", side_effect=mock_side_effect
    ) as mock_post, patch(
        "requests.Session.delete", side_effect=mock_side_effect
    ) as mock_delete, patch(
        "requests.Session.put", side_effect=mock_side_effect
    ) as mock_put:
        yield {
            "get": mock_get,
            "post": mock_post,
