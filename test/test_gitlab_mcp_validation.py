...
def test_mcp_server_up():
    ...
    try:
        ...
        if response.status_code == 404: # Try root if /mcp/messages not valid
             response = requests.post(f"http://localhost:8005/mcp", json=payload, headers=HEADERS)
        ...
        if response.status_code != 200:
             ...
             logger.warning(f"Initial check failed: {response.status_code}. Trying alternate endpoints.")
             return False
