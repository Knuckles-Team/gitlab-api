...
def check_container_logs(container_name):
    ...
    try:
        ...
        for line in logs.splitlines():
            ...
            if "traceback" in lower_line or "exception" in lower_line or ("error" in lower_line and "debug" not in lower_line):
...
def test_graphiti_ingestion():
    ...
    assert text is not None, f"No response text from assistant. Result: {json.dumps(result, indent=2)}"
...
def test_branch_creation_flow():
    ...
    assert text_create is not None, f"No response text from assistant. Result: {json.dumps(result_create, indent=2)}"
    ...
    assert text_verify is not None, f"No response text from assistant. Result: {json.dumps(result_verify, indent=2)}"
...
def test_pipeline_flow():
    ...
    assert text_run is not None, f"No response text from assistant. Result: {json.dumps(result_run, indent=2)}"
