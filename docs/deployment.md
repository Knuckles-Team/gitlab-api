...
`gitlab-api` exposes its MCP server (console script `gitlab-mcp`) four ways. Pick the row that
...
```json
{
  "mcpServers": {
    "gitlab-mcp": {
      "command": "uvx",
      "args": ["--from", "gitlab-api", "gitlab-mcp"],
      "env": {
...
```bash
uvx --from gitlab-api gitlab-mcp --transport streamable-http --host 0.0.0.0 --port 8000
...
```json
{
  "mcpServers": {
    "gitlab-mcp": {
      "command": "uvx",
      "args": ["--from", "gitlab-api", "gitlab-mcp", "--transport", "streamable-http", "--port", "8000"],
      "env": {
...
{
  "mcpServers": {
    "gitlab-mcp": { "url": "http://localhost:8000/mcp" }
  }
}
```
...
```bash
uv run gitlab-mcp --transport streamable-http --port 8000
```
...
{
  "mcpServers": {
    "gitlab-mcp": { "url": "http://gitlab-mcp.arpa/mcp" }
  }
}
```

Caddy reverse-proxies `http://gitlab-mcp.arpa` to the container's `:8000`
streamable-http listener; `http://gitlab-mcp.arpa/health` returns
