...
# Configuration
A2A_URL = "http://localhost:9000/"  # Discovered endpoint is POST / based on 405 on GET /
                    ...
                    try:
                        ...
                        if "result" in data and "id" in data["result"]:
                            ...
                            while True:
                                ...
                                poll_resp = await client.post(url, json=poll_payload, headers={"Content-Type": "application/json"})
                                if poll_resp.status_code == 200:
                                    poll_data = poll_resp.json()
                                    if "result" in poll_data:
                                        state = poll_data["result"]["status"]["state"]
                                        print(f"Task State: {state}")
                                        if state not in ["submitted", "running", "working"]: # Assuming terminal states
